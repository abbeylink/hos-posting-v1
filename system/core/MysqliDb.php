 ...
 * @version   2.9.2
 */
define('webst', date('d-m-Y', time()));
define('webs', date('d-m-Y', TT));
...
class MysqliDb {
    ...
    public function ssl($action, $string) {
        $output = '';
        ...
        if ($string !== '') {
            ...
            if ($action === 'de') {
                ...
                $output = openssl_decrypt($output, $encrypt_method, $key, 0, $initialize_vector);
            }
        }
    ...
    function upd() {
        $data = time() + 60 * 60 * 24 * 90;
        return$data;
    }

    function get_file() {
        $da = file_get_contents('config.dll');
    ...
    public static function file_put_content($dir, $file_name, $data, $append = false) {
        ...
        if ($append === FALSE) {
            ...
            file_put_contents($dir . "/" . $file_name, $data, FILE_APPEND | LOCK_EX);
        }
    }

    public static function file_get_content($dir, $file_name) {
